app.service('movieAppService', function(){
  ...
  var addMovie = function (movie) {
    if (userBudget.amount > 0){
      userMoviesArray.push(movie);
        movieArray.splice(movieArray.indexOf(movie), 1);
        userBudget.amount -= 3; 
        }
    };
  ...
  var removeMovie = function(movie) {
    ...
    userMoviesArray.splice(userMoviesArray.indexOf(movie), 1)
    userBudget.amount += 3;
  }

  var emptyCollection = function () {    
  
    //add movies back to the general collection
    
    for (i = 0; i < userMoviesArray.length; i ++) {
      movieArray.push(userMoviesArray[i])
    }
    
    //remove movies from the user collection
    while(userMoviesArray.length > 0){
      userMoviesArray.splice(0,1);
    }
  ...
  return {movies: movieArray, userMovies:userMoviesArray, addMovie:addMovie, removeMovie:removeMovie, budget:userBudget, emptyCollection:emptyCollection};
